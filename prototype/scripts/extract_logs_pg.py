...
def main():
    ...
    finished_pg_token = "finished finding parity computation scheme"
    ...
    with open(log_path, "r") as f:
        for line in f.readlines():
            if finished_pg_token in line:
                match = re.search(r"finished finding parity computation scheme.*, time: (\d+.\d+)", line)
                if not match or not match.groups():
                    print("Error: cannot find pg_time")
                    exit()
                # print(line)
                pg_time = float(match.group(1))

                # append results
                baseline_results[baselines[baseline_cnter]]["pg_time"].append(pg_time)
    ...
    for bl_name, bl_result in baseline_results.items():
        ...
        bl_result["pg_time"] = student_t_dist(bl_result["pg_time"])

        # polish (mean, lower, upper)
        bl_result["pg_time"] = bl_result["pg_time"][0], bl_result["pg_time"][0] + bl_result["pg_time"][1], bl_result["pg_time"][0] - bl_result["pg_time"][1]

        # polish (max_load to second)
        # bl_result["pg_time"] = [item * 1.0 / 1000 for item in bl_result["pg_time"]]
