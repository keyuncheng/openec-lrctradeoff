...
def main():
    ...
    finished_pg_token = "finished finding block relocation scheme"
    ...
    with open(log_path, "r") as f:
        for line in f.readlines():
            if finished_pg_token in line:
                match = re.search(r"finished finding block relocation scheme.*, time: (\d+.\d+)", line)
                if not match or not match.groups():
                    print("Error: cannot find rd_time")
                    exit()
                # print(line)
                rd_time = float(match.group(1))

                # append results
                baseline_results[baselines[baseline_cnter]]["rd_time"].append(rd_time)
    ...
    for bl_name, bl_result in baseline_results.items():
        ...
        bl_result["rd_time"] = student_t_dist(bl_result["rd_time"])

        # polish (mean, lower, upper)
        bl_result["rd_time"] = bl_result["rd_time"][0], bl_result["rd_time"][0] + bl_result["rd_time"][1], bl_result["rd_time"][0] - bl_result["rd_time"][1]

        # polish (max_load to second)
        # bl_result["rd_time"] = [item * 1.0 / 1000 for item in bl_result["rd_time"]]
