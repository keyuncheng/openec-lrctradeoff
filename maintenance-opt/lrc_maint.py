...
def main():
    ...
    try:
        ...
        # a(i,j): number of data blocks stored in rack R_i from the local
        # group G_j.  Non-negative integer
        a = model.addVars(max_num_racks, ecl, vtype=GRB.INTEGER, lb=0, name="a")

        # b(i,j): number of local parity blocks stored in rack R_i from the
        # local group G_j.  Binary value
        ...
        # Z: number of racks the entire stripe spans （data + local parity + global parity)
        Z = model.addVar(vtype=GRB.INTEGER, lb=0, ub=eck+ecl, name="Z")
        model.addConstr(Z == I_b.sum('*') + 1)
