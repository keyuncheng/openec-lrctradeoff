...
int ECDAG::findCluster(vector<int> childs)
{
  ...
  {
    if (_clusterMap[i]->childsInCluster(childs))
      return i;
  }
  return -1;
}
...
int ECDAG::BindX(vector<int> idxs)
{
  ...
  for (int i = 0; i < childnodes.size(); i++)
    childids.push_back(childnodes[i]->getNodeId());
...
vector<int> ECDAG::toposort()
{
  ...
  while (true)
  {
    if (inNum2List.find(0) == inNum2List.end())
      break;
    if (inNum2List[0].size() == 0)
      break;
                      ...
                      bool locality)
{
  ...
  { // add support for optimization 3
    unordered_map<int, string> cid2Rack;
    for (auto item : _ecNodeMap)
    {
      int cid = item.first;
    ...
    // after we run Opt3, we need to recheck ip in cid2ip
    vector<int> toposeq = toposort();
    for (int i = 0; i < toposeq.size(); i++)
    {
      int curcid = toposeq[i];
  ...
  for (int clusteridx = 0; clusteridx < _clusterMap.size(); clusteridx++)
  {
    ...
    for (int i = 0; i < curChilds.size(); i++)
    {
      ...
      if (subchilds.find(r) == subchilds.end())
      {
        ...
        subchilds[r].push_back(curChilds[i]);
    }
    if (ECDAG_DEBUG_ENABLE)
      cout << "ECDAG::childs are sorted into " << subchilds.size() << " racks" << endl;

    if (numoutput == 1)
    {
      // we deploy pipelining technique for current cluster
      if (ECDAG_DEBUG_ENABLE)
        cout << "numoutput == 1, deploy pipelining optimization" << endl;
      int parent = curParents[0];
      ECNode *parentnode = _ecNodeMap[parent];
      bool isProot = false;
      if (find(_ecHeaders.begin(), _ecHeaders.end(), parent) != _ecHeaders.end())
        isProot = true;
    ...
    else
    {
      ...
      for (auto item : subchilds)
      {
        ...
        if (itemchilds.size() > numoutput)
        {
          update = true;
          if (ECDAG_DEBUG_ENABLE)
            cout << "inputsize = " << itemchilds.size() << ", outputsize = " << numoutput << ", there is space for optimization" << endl;
          ...
          if (ECDAG_DEBUG_ENABLE)
          {
            ...
            for (auto cid : subparents)
              cout << cid << " ";
            cout << ")" << endl;
            cout << "updated globalchilds: ( ";
            for (auto cid : globalChilds)
              cout << cid << " ";
            cout << ")" << endl;
          }
          ...
          for (int i = 0; i < numoutput; i++)
          {
            ...
            {
              if (j == i)
                globalCoefs[parent].push_back(1);
              else
                globalCoefs[parent].push_back(0);
            }
          }
      ...
      for (auto c : globalChilds)
      {
        ...
        if (inroot != _ecHeaders.end())
          _ecHeaders.erase(inroot);
      }
    }
  }
...
void ECDAG::Opt3(unordered_map<int, string> n2Rack)
{
  // 1. now we have all cids corresponding racks, iterate through all clusters
  vector<int> deletelist;
  for (int clusteridx = 0; clusteridx < _clusterMap.size(); clusteridx++)
  {
    ...
    if (numoutput == 1)
    { // Only fix for single parent
      // we deploy hierarchical repair for current cluster
      if (ECDAG_DEBUG_ENABLE)
        cout << "numoutput == 1, deploy hierarchical optimization" << endl;
      int parent = curParents[0];
      ECNode *parentnode = _ecNodeMap[parent];
      bool isProot = false;
      if (find(_ecHeaders.begin(), _ecHeaders.end(), parent) != _ecHeaders.end())
      ...
      // case 1: parent and child are in different racks
      for (auto group : subchilds)
      {
        vector<int> data;
        vector<int> coef;
        ...
        for (auto c : group.second)
        {
          data.push_back(c);
          int curcoef = parentnode->getCoefOfChildForParent(c, parent);
          coef.push_back(curcoef);
        }
        if (data.size() > 1)
        { // at least two childs in the rack
          int cr_vs_id = _optId++;
          Join(cr_vs_id, data, coef);
          BindY(cr_vs_id, data[0]);

          // add cross-rack symbols
          cr_symbols.push_back(cr_vs_id);
          cr_coefs.push_back(1);
        }
        else
        {
          cr_symbols.push_back(data[0]);
          cr_coefs.push_back(coef[0]);
        }
      }

      // case 2: parent and child are in the same rack (prack)
      int bindY_idx = -1;
      if (subchilds.find(prack) != subchilds.end())
      {
        vector<int> data;
        ...
        if (data.size() > 1)
        {
          int cr_vs_id = _optId++;
          Join(cr_vs_id, data, coef);
          BindY(cr_vs_id, data[0]); // collocate with parent
      ...
      Join(parent, cr_symbols, cr_coefs);
      if (bindY_idx != -1)
      {
        BindY(parent, bindY_idx);
      }
                                                   ...
                                                   unordered_map<int, pair<string, unsigned int>> objlist)
{
  ...
  for (int i = 0; i < sortedList.size(); i++)
  {
    ...
    if (cmd)
      agCmds.insert(make_pair(cidx, cmd));
  }
  ...
  if (w == 1)
  {
    ...
    for (auto item : agCmds)
    {
      ...
      for (int i = 0; i < prevCids.size(); i++)
      {
        ...
        if (agCmds.find(childCid) == agCmds.end())
          continue;
    ...
    for (auto item : tomerge)
    {
      ...
      for (auto item : readCidListRef)
        mergeref.insert(item);
      for (auto item : computeCidRef)
        mergeref.insert(item);
