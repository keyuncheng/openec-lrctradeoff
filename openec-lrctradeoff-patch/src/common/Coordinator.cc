...
void Coordinator::registerFile(CoorCommand *coorCmd)
{
  ...
  if (mode == 0)
    registerOnlineEC(clientIp, filename, ecid, filesizeMB);
  else if (mode == 1)
...
void Coordinator::registerOnlineEC(unsigned int clientIp, string filename, string ecid, int filesizeMB)
{
  ...
  for (int i = 0; i < ecn; i++)
  {
    ...
    {
      if (find(candidates.begin(), candidates.end(), clientIp) != candidates.end())
        curIp = clientIp;
      else
        curIp = chooseFromCandidates(candidates, _conf->_data_policy, "data");
    }
...
void Coordinator::registerOfflineEC(unsigned int clientIp, string filename, string ecpoolid, int filesizeMB)
{
  ...
  for (int objidx = 0; objidx < objnum; objidx++)
  {
    ...
    vector<int> colocWith;
    if (idx2group.find(stripeidx) != idx2group.end())
      colocWith = idx2group[stripeidx];
...
vector<unsigned int> Coordinator::getCandidates(vector<unsigned int> placedIp, vector<int> placedIdx, vector<int> colocWith)
{
    ...
    if (placedIp.size() > curIdx)
    {
      ...
      for (auto item : _conf->_rack2Ips[rack])
      {
        if (find(toret.begin(), toret.end(), item) == toret.end() &&
            find(placedIp.begin(), placedIp.end(), item) == placedIp.end())
  ...
  if (toret.size() == 0)
  {
    ...
    {
      if (find(placedIp.begin(), placedIp.end(), item) == placedIp.end())
        toret.push_back(item);
    }
  }
  return toret;
...
void Coordinator::offlineEnc(CoorCommand *coorCmd)
{
  ...
  unsigned long long basesizeBytes = (unsigned long long)basesizeMB * 1048576;
  int pktnum = basesizeBytes / (unsigned long long)_conf->_pktSize;
...
void Coordinator::getFileMeta(CoorCommand *coorCmd)
{
  ...
  memcpy(filemeta + metaoff, (char *)&tmpr, 4);
  metaoff += 4;
  // 1.2 filesizeMB
  int tmpsize = htonl(filesizeMB);
  memcpy(filemeta + metaoff, (char *)&tmpsize, 4);
  metaoff += 4;
  ...
  if (redundancy == 0)
  {
    ...
    // ecn|eck|ecw|
    int tmpn = htonl(ecn);
    memcpy(filemeta + metaoff, (char *)&tmpn, 4);
    metaoff += 4;
    int tmpk = htonl(eck);
    memcpy(filemeta + metaoff, (char *)&tmpk, 4);
    metaoff += 4;
    int tmpw = htonl(ecw);
    memcpy(filemeta + metaoff, (char *)&tmpw, 4);
    metaoff += 4;
  ...
  else
  {
    ...
    for (auto objname : objlist)
    {
      ...
      memcpy(filemeta + metaoff, (char *)&tmpobjlen, 4);
      metaoff += 4;
      memcpy(filemeta + metaoff, objname.c_str(), len);
      metaoff += len;
    }
...
void Coordinator::onlineDegradedInst(CoorCommand *coorCmd)
{
  ...
  for (int i = 0; i < eck; i++)
  {
    if (integrity[i] == 0)
      continue;
    else if (find(loadidx.begin(), loadidx.end(), i) == loadidx.end())
      loadidx.push_back(i);
  }
...
void Coordinator::optOfflineDegrade(string lostobj, unsigned int clientIp, OfflineECPool *ecpool, ECPolicy *ecpolicy)
{
  ...
  for (int i = 0; i < stripeobjs.size(); i++)
  {
    ...
    unsigned int loc;
    if (integrity[i] == 1)
      loc = ssentry->getLocOfObj(objname);
    else
      loc = clientIp;
    pair<string, unsigned int> curpair = make_pair(objname, loc);
  ...
  memcpy(instruction + offset, (char *)&tmpstripenamelen, 4);
  offset += 4;
  memcpy(instruction + offset, stripename.c_str(), stripename.length());
  offset += stripename.length();
  int tmpnum = htonl(rootinfo.size());
  memcpy(instruction + offset, (char *)&tmpnum, 4);
  offset += 4;
  for (int i = 0; i < rootinfo.size(); i++)
  {
    ...
    memcpy(instruction + offset, (char *)&tmpcidx, 4);
    offset += 4;
    memcpy(instruction + offset, (char *)&tmpip, 4);
    offset += 4;
  }
